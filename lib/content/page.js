...
export default function prepPage(meta, options, isDev) {
  ...
  return {
    ...
    set toc(entry) {
      const { permalink } = this
      if (typeof toc[permalink] === 'undefined') {
        toc[permalink] = {
          topLevel: Infinity,
          items: {}
        }
      }
      if (typeof toc[permalink].items[entry.slug] !== 'undefined') {
        return
      }
      ...
      if (tocEntry.level < toc[permalink].topLevel) {
        toc[permalink].topLevel = tocEntry.level
      }
      if (typeof toc[permalink].items[entry.slug] === 'undefined') {
        toc[permalink].items[entry.slug] = tocEntry
      }
    },

    get toc() {
      const { permalink } = this
      console.log('Trying to get to for ' + permalink)
      return toc[permalink]
    }
  }
}
