    ...
    get body() {
      if (isDev || !cached.body) {
        ...
        if (fileName.search(/\.comp\.md$/) > -1) {
          ...
          if (options.markdown.plugins.toc) {
            // Inject callback in markdown-it-anchor plugin
            options.markdown.plugins.toc[1].callback = (token, info) =>
              parserCallbackForToc(this, token, info)
          }
          cached.body = options.parser.render(_rawData.body) // markdown to html
    ...
    get _rawData() {
      if (isDev || !cached.data) {
        ...
        if (meta.fileName.search(/\.md$/) > -1) {
          const { data: attributes, content: body } = matter(source.toString())
