...
export default function prepPage(meta, options, isDev) {
  ...
  return {
    ...
    set toc(entry) {
      ...
      if (tocEntry.level < cached.toc.topLevel) {
        cached.toc.topLevel = tocEntry.level
      }
      if (typeof cached.toc.items[entry.slug] === 'undefined') {
        cached.toc.items[entry.slug] = tocEntry
      }
    },
