...
export default function prepPage(meta, options, isDev) {
  ...
  return {
    ...
    get attributes() {
      ...
      return { ...this._rawData.attributes, ...{ toc: cached.toc } }
    },
    ...
    set toc(payload) {
      ...
      if (
        payload.Btoken.level < cached.toc.topLevel &&
        payload.Btoken.level !== 0
      ) {
        cached.toc.topLevel = payload.Btoken.level
      }
      const tocEntry = {
        level: payload.Btoken.level,
        title: payload.Binfo.title,
        link: '#' + payload.Binfo.slug
      }
      if (typeof cached.toc.items[payload.Binfo.slug] === 'undefined') {
        cached.toc.items[payload.Binfo.slug] = tocEntry
      }
    },
