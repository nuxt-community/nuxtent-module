...
const curryResponseHandler = (api, endpoint, contentPath, dirOpts, isDev) => {
  const db = createDatabase(contentPath, endpoint, dirOpts, isDev)
...
const createRouter = (api, options) => {
  const router = Router()
  const { contentPath, content, parsers, isDev } = options
