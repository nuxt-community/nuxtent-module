      ...
      if (routePaths.has(route.name)) {
        ...
        route.children.forEach(nestedRoute => {
          if (routePaths.has(nestedRoute.name)) {
            ...
            if (isOptional) { nestedRoute.path = routePaths.get(nestedRoute.name) + '?' } else nestedRoute.path = routePaths.get(nestedRoute.name)
          }
        })
      }
    })
...
function addRoutes (nuxtOpts, routeData) {
  ...
  if (Array.isArray(routes)) nuxtOpts.generate.routes = routes.concat(routeData)
