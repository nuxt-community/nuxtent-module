    ...
    dirOpts.routes.forEach(route => {
      ...
      switch (route.method) {
        case 'get':
          ...
          dynamicRoutePaths.set(route.name, dirOpts.permalink)
          findAll().forEach((page) => {
            routeData.push({ route: pathPrefix + page.permalink, payload: page })
            assetMap.set(buildPath(page.permalink, dirName, options), page)
          })
          break
        case 'getAll':
          routeData.push({ route: route.name, payload: findAll() })
...
function addRoutes (nuxtOpts, generateRoutes) {
  ...
  if (isArray(routes)) nuxtOpts.generate.routes = routes.concat(generateRoutes)
...
function getPrefix (routeName) {
  const [match, prefix] = routeName.match(/(^[a-zA-Z]*)(-)/) // matches `prefix-`
  return prefix
    ? join('/', prefix) // nested route
    : '/' // top level route
}
