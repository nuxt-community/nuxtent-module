      ...
      if (routePaths.has(route.name)) {
        ...
        route.children.forEach(nestedRoute => {
          if (routePaths.has(nestedRoute.name)) {
            ...
            let overwritedPath = routePaths.get(nestedRoute.name)
            const match = overwritedPath.match(/\/(.*)/)
            if (match) {
              overwritedPath = match[1]
            }
            if (isOptional) {
              nestedRoute.path = overwritedPath + '?'
            } else {
              nestedRoute.path = overwritedPath
            }
          }
...
const buildContent = (nuxt, buildDir, isStatic, options) => {
  const { contentDir, content, isDev } = options
  ...
  Object.keys(content).forEach(dirName => {
    ...
    if (generate && isStatic) {
      const db = createDatabase(contentDir, dirName, content[dirName], isDev)
