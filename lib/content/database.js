...
const globAndApply = (dirPath, fileStore, applyFn, nestedPath = '/') => {
  ...
  stats.forEach((stat, index) => {
    ...
    if (statSync(statPath).isFile()) {
      ...
      if (stat.search(/\.(yaml|yml|md|json)$/) !== -1) {
 ...
 *
 * @export
 * @param {String} contentDir ~/content
 * @param {String} dirName El nombre del directorio del contenido
 * @param {any} dirOpts Las opciones del contenido
 * @returns {Object} Una clase de base de datos
 */
export default function createDatabase(contentDir, dirName, dirOpts) {
  ...
  const pagesMap = globAndApply(
    ...
    ({ index, fileName, section }, store) => {
      ...
      const lazyPage = prepPage(meta, dirOpts, parser)
