...
// import consola from '../util/debug'
 ...
 */
export default async (moduleOptions = {}, options) => {
  ...
  const nuxtentUserConfig = await loadNuxtentConfig(options.rootDir)
  /**
  ...
  logger.debug('Config finished', JSON.stringify(nuxtentConfig, ' ', 1))
  return Promise.resolve(nuxtentConfig)
}
