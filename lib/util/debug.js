...
const NS_SEPARATOR = chalk.red(figures(' › '))
...
class DebugReporter {
  ...
  log(logObj) {
    ...
    if (logObj.badge) {
      this.stream.write(
        '\n\n' +
          this.formatBadge(logObj.type, logObj.color, icon) +
          message +
          '\n\n'
      )
    } else {
      this.stream.write(
        this.formatTag(logObj.type, logObj.color, icon) + message + '\n'
      )
    }

    if (logObj.additional) {
      // const lines = JSON.stringify(logObj.additional)
      //   .split('\n')
      //   .map(s => '  ' + s)
      //   .join('\n')

      this.stream.write(JSON.stringify(JSON.parse(logObj)) + '\n')
    }
  }
}
