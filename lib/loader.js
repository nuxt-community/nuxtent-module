...
const mdComponents = (source, componentsDir, extensions) => {
  ...
  while ((result = componentExpression.exec(transformedSource))) {
    ...
    if (!codeSnippet) {
      ...
      transformedSource = transformedSource.replace(match, `<${componentName} ${props || ''} />`)
    }
  }
...
module.exports = function (source) {
  ...
  const { transformedSource, components } = mdComponents(body, componentsDir, extensions)
