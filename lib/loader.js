...
consola.add(new consola.FancyReporter())
...
const mdComponents = (source, componentsDir, extensions) => {
  ...
  const componentExpression = new RegExp(
    [
      ...
      '@\\[(#)?(\\/)?([\\w/\\-_\\\\]*?)\\](?:\\((.*?)\\))?(:[\\w/\\-_]*)?' // markdown component - '@[]' or '@[]()' or '@[#]():n' or @[/]
    ].join('|'),
    'g'
  )
  ...
  while ((result = componentExpression.exec(transformedSource))) {
    ...
    if (!codeSnippet) {
      ...
      if (closeSlot) {
        logger.info('Encontrado cierre de Slot')
        console.log({
          match,
          codeSnippet,
          isSlot,
      ...
      if (isSlot) {
        logger.info('Encontrado componente con Slot')
        console.log({
          match,
          codeSnippet,
          isSlot,
