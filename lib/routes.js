 ...
 *
 */
exports.interceptRoutes = function (routes, content, options) {
  ...
  options.dirs.forEach(registeredType => {
    ...
    const routeName = dirOpts.route || options.route
 ...
 */
exports.getRoutes = function (content) {
  const contentRoutes = []
  Object.keys(content).forEach(registeredDir => {
    content[registeredDir].forEach(page => {
      contentRoutes.push(page.data.permalink)
    })
  })
