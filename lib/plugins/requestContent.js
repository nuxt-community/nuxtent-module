...
const serializePermalink = permalink => {
  const allButFirstSlash = /(?!^\/)(\/)/g
  return permalink.replace(allButFirstSlash, '.')
}

export default ({ app, isServer, isClient, isStatic, hotReload, route }) => {
  ...
  async function fetchContent(path, permalink, query = '') {
    if (isAPI) {
      ...
      const serializedPermalink = serializePermalink(permalink)
