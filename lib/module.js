...
export default function ContentModule(moduleOpts) {
  ...
  const routerOptions = {
    contentPath: join(options.sitePath, options.srcDir),
    content: {
      ...options.content
    },
    parsers: {
  ...
  const { srcDir, buildDir, api } = options
  ...
  buildContent(this, buildDir, routerOptions)

  // 2. Add content API
  const router = createRouter(api, routerOptions)
