...
const nuxtdownConfig = rootDir => {
  const rootConfig = join(rootDir, 'nuxtdown.config.js')
  try {
    ...
    throw new Error(`[Invalid Nuxtdown configuration] ${err}`)
  }
}
...
export default function ContentModule(moduleOpts) {
  const userOptions =
    nuxtdownConfig(this.options.rootDir) || this.options.nuxtdown || {}
  ...
  this.nuxt.hook('build:before', builder => {
    logger.info('starting build nuxtdown')
    ...
    this.requireModule([
      '@nuxtjs/axios',
      {
        ...
        browserBaseURL:
          (apiOptions.browserBaseURL === '/' ? '' : apiOptions.browserBaseURL) +
