 ...
 * @property {Object.<string, [module|function, Object]>} plugins The plugins to use on markdown,
 ...
 * @property {?function} parser The specific markdown configuration for this container
 ...
 * @property {Array.<string>} loaderComponentExtensions The parser instance
 */
...
/**
 * @typedef {Map.<string, string>} NuxtentRoutePaths
 */
...
/**
 * Creates a parser based on the config for the container
 * @param {NuxtentConfigMarkdown} markdownConfig The container config
 * @returns {Function} The markdown parser instance
 */
...
export default class NuxtentConfig {
   ...
   */
    ...
    return moduleContianer.extendRoutes(
       ...
       * @returns {void}
       */
      (routes) => routes.forEach(renameRoutePath)
    )
  }
  ...
  createContentDatabase() {
    ...
    this.rawContent.forEach(([dirName, content]) => {
      const db = new Database(this.build.contentDir, dirName, content)
